...
export function groupPageTree(tree: PageTree.Root, locale: Locale = defaultLocale): PageTree.Root {
  ...
  for (const node of children) {
    if (node.type === 'page') {
      ...
      if (introFolders.includes(segment)) {
        ...
        devNodes.push(node);
      } else if (uiFolders.includes(segment)) {
        uiNodes.push(node);
  ...
  const virtualIntroFolder = {
    type: 'folder',
    name: locale === 'pt' ? 'Introdução e Guias' : 
          locale === 'es' ? 'Introducción y Guías' : 
          locale === 'fr' ? 'Introduction & Guides' : 
          locale === 'de' ? 'Einführung & Anleitungen' : 'Introduction & Guides',
    root: true,
  ...
  const virtualHostFolder = {
    type: 'folder',
    name: locale === 'pt' ? 'Auto-Alojamento' : 
          locale === 'es' ? 'Auto-Hospedaje' : 
          locale === 'fr' ? 'Auto-Hébergement' : 
          locale === 'de' ? 'Self-Hosting' : 'Self-Hosting',
    root: true,
  ...
  const virtualDevFolder = {
    type: 'folder',
    name: locale === 'pt' ? 'Desenvolvimento' : 
          locale === 'es' ? 'Desarrollo' : 
          locale === 'fr' ? 'Développement' : 
          locale === 'de' ? 'Entwicklung' : 'Development',
    root: true,
    index: devFolderNode?.index,
    children: devNodes,
    icon: <Cpu size={16} />,
    description: locale === 'pt' ? 'Arquitetura do sistema, APIs, app móvel e IA' : 'System architecture, backend APIs, mobile app, and AI',
  } as unknown as PageTree.Folder;
  
  const uiFolderNode = uiNodes.find(n => n.type === 'folder') as PageTree.Folder | undefined;
  const virtualUIFolder = {
    type: 'folder',
    name: 'Cookest UI',
    root: true,
    index: uiFolderNode?.index,
    children: uiNodes,
    icon: <Layers size={16} />,
    description: locale === 'pt' ? 'Bibliotecas de componentes UI partilhadas' : 'Shared UI component libraries',
  } as unknown as PageTree.Folder;
  ...
  if (virtualUIFolder.children.length > 0) {
    finalChildren.push(virtualUIFolder);
  }
