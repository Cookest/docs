...
Cookest now uses two backend services with separate startup migrations:

- the **Food API** owns the catalog database tables
- the **App API** owns the authenticated app tables

Both use **PostgreSQL 15+** accessed via **SeaORM 1.1**. Migrations run automatically at server startup — all operations use `IF NOT EXISTS` making them safe to replay.
...
<Callout type="info">
The migration SQL in `api/crates/food-api/src/main.rs` and `api/crates/app-api/src/main.rs` is the canonical schema. This document reflects that schema — consult the source files for exact column types and constraints.
</Callout>
